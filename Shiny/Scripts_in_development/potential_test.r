...
################################################################################ this works with only ecoregion
################ and iceas areas from the map selection
...
shinyApp(
    ui = fluidPage(
        ...
        selectizeInput(
            inputId = "StockKeyLabel",
            label = "StockKeyLabel",
            choices = unique(stock_list_all$StockKeyLabel),
            selected = NULL,
            multiple = TRUE,
            #placeholder = 'select ICES Area(s)'
        ),
    ...
    server <- function(input, output, session) {
        ...
        output$tbl_summary <- DT::renderDT({
          ...
          req(selected_1$groups, selected_2$groups, input$StockKeyLabel)
          stock_list_eco <- stock_list_all %>% filter(str_detect(EcoRegion, selected_1$groups))
          stock_list_area <- stock_list_eco %>% filter(str_detect(ICES_area, selected_2$groups))
          stock_list_stock_name <- stock_list_area %>% filter(str_detect(StockKeyLabel, input$StockKeyLabel))
          stock_list_stock_name
          # if (length(selected_1$groups) > 0) {
          #   stock_list_all %>% filter(str_detect(EcoRegion, selected_1$groups))
          # #}
          # } else if (length(selected_2$groups) > 0) {
          #   stock_list_all %>% filter(str_detect(ICES_area, selected_2$groups))
          # }

          # stock_list_all %>% filter(str_detect(EcoRegion, selected_1$groups) & str_detect(ICES_area, selected_2$groups) & str_detect(StockKeyLabel, input$StockKeyLabel))
          #this does not work well cause it waits for the last fileter element to start filtering
        }
