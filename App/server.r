...
server <- function(input, output, session) {
  ...
  # # Identify “latest year” as “the default selected year at startup”
  # default_year <- reactiveVal(NULL)
  # observeEvent(TRUE,
  #   {
  #     default_year(isolate(input$selected_years))
  #   },
  #   once = TRUE,
  #   query$active_year <- input$selected_years
  # )
  # values of the query string and first visit flag
  query <- reactiveValues(query_from_table = FALSE, active_year = NULL)

  # set active year whenever the input changes
  ...
  # capture default year once, at startup
  default_year <- reactiveVal(NULL)
  observeEvent(input$selected_years,
    {
      default_year(isolate(input$selected_years))
  ...
  is_latest_selected <- reactive({
    req(default_year(), input$selected_years)
    identical(input$selected_years, default_year())
  })
  ...
  #   # Always start from SID metadata (ecoregion filtering relies on it)
  #   sid_dt <- getSID_meta_cached(year)

  #   # Attach advice keys/components
  #   if (is_latest_selected()) {
  #     # Fast path: 1 call total for advice mapping
  #     sag_latest <- getSAG_latest_cached()
  #     data.table::setkey(sid_dt, StockKeyLabel)
  #     data.table::setkey(sag_latest, StockKeyLabel)
  #     sid_dt <- sag_latest[sid_dt] # left join onto SID
  #   } else {
  #     # Historical path:
  #     # For the selected SID year, SID contains YearOfLastAssessment per stock.
  #     # We fetch SAG StockList for only the years present in YearOfLastAssessment, then join by both fields.
  ...
  #     # Join on StockKeyLabel + YearOfLastAssessment = AssessmentYear
  #     data.table::setkey(sid_dt, StockKeyLabel)
  #     data.table::setkey(sag_all, StockKeyLabel, AssessmentYear)
  #     sid_dt <- sag_all[sid_dt]
  #   }

  #   # Drop rows with no advice key (your table depends on it)
  #   sid_dt <- sid_dt[!is.na(AssessmentKey)]

  #   # Ecoregion filter (your original logic)
  #   out <- purrr::map_dfr(
  #     .x = input$selected_locations,
  #     .f = function(.x) sid_dt %>% dplyr::filter(stringr::str_detect(EcoRegion, .x))
  #   )

  #   if (nrow(out) == 0) {
  #     return(out)
  #   }
  #   out %>% dplyr::arrange(StockKeyLabel)
  # }) %>%
  #   bindCache(input$selected_locations, input$selected_years) %>%
  #   bindEvent(input$selected_locations, input$selected_years)
  eco_filter <- reactive({
  ...
  # advice_view_info <- reactive({
  #   # asd_record <- getAdviceViewRecord(assessmentKey = query$assessmentkey)
  #   asd_record <- getASD_by_key_cached(assessment_key = query$assessmentkey)
  ...
  # })
  advice_view_info <- reactive({
    req(query$assessmentkey, query$active_year)
    asd_record <- getASD_by_key_cached(query$assessmentkey)
